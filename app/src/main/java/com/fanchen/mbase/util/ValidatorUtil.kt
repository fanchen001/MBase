...
object ValidatorUtil {
    ...
    val CODE_BANK = mapOf("SRCB" to "深圳农村商业银行",
            "BGB" to "广西北部湾银行",
            "SHRCB" to "上海农村商业银行",
            "BJBANK" to "北京银行",
            "WHCCB" to "威海市商业银行",
            "BOZK" to "周口银行",
            "KORLABANK" to "库尔勒市商业银行",
            "SPABANK" to "平安银行",
            "SDEB" to "顺德农商银行",
            "HURCB" to "湖北省农村信用社",
            "WRCB" to "无锡农村商业银行",
            ...
            "JNBANK" to "济宁银行",
            "CMB" to "招商银行",
            "JINCHB" to "晋城银行JCBANK",
            "FXCB" to "阜新银行",
            "WHRCB" to "武汉农村商业银行",
            "HBYCBANK" to "湖北银行宜昌分行",
            "TZCB" to "台州银行",
            "TACCB" to "泰安市商业银行",
            "XCYH" to "许昌银行",
            "CEB" to "中国光大银行",
            ...
            "QDCCB" to "青岛银行",
            "HKBEA" to "东亚银行",
            "HBHSBANK" to "湖北银行黄石分行",
            "WZCB" to "温州银行",
            "TRCB" to "天津农商银行",
            "QLBANK" to "齐鲁银行",
            "GDRCC" to "广东省农村信用社联合社",
            "ZJTLCB" to "浙江泰隆商业银行",
            "GZB" to "赣州银行",
            "GYCB" to "贵阳市商业银行",
            "CQBANK" to "重庆银行",
     ...
     * 包括正整数，负整数
     */
    fun isAllNumber(input: CharSequence): Boolean {
        return isMatch(REGEX_INTEGER, input) || isMatch(REGEX_NOT_NEGATIVE_INTEGER, input) || isMatch(REGEX_NOT_POSITIVE_INTEGER, input)
    }
    ...
    fun isNumber(input: CharSequence): Boolean {
        return isAllNumber(input) || isFloatNumber(input)
    }
     ...
     * 包括正浮点数，负浮点数
     */
    fun isFloatNumber(input: CharSequence): Boolean {
        return isMatch(REGEX_POSITIVE_FLOAT, input) || isMatch(REGEX_NEGATIVE_FLOAT, input)
    }
    ...
    fun getName(cardnumber: CharSequence): String? {
        val charBin = cardnumber.substring(0, 6)
        var bin = 0
        var index = 0
        bin = Integer.valueOf(charBin)
        index = binarySearch(BANKBIN, bin)
        return if (index == -1 || index > BANKNAME.size) {
            null
        } else BANKNAME[index].substring(0, 4)

    }

    // 查找方法
    fun binarySearch(srcArray: IntArray, des: Int): Int {
        var low = 0
        val high = srcArray.size
        while (low < high) {
 ...
 */
    ...
    fun getBankCardCheckCode(nonCheckCodeCardId: String?): Char {
        if (nonCheckCodeCardId == null || nonCheckCodeCardId.trim { it <= ' ' }.isEmpty() || !nonCheckCodeCardId.matches("\\d+".toRegex())) {
        ...
        return if (luhmSum % 10 == 0) '0' else (10 - luhmSum % 10 + '0'.toInt()).toChar()  //这边+'0'，不是拼接，在Java和C#中是8+0的ASCII码得到8在ASCII中的编码值，然后通过(char)转成字符'8'
    }
}